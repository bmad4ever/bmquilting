...
The diagram below summarises the heuristic process.
...
# Default behaviour: FFT-only analysis
block_size = bmquilting.guess_nice_block_size(image)

# Select a heuristic based on requirements:
...
This analysis identifies dominant periodicities in the texture by analysing its magnitude spectrum in the frequency domain.
...
3.  **Wavelength Calculation**: For each peak, the frequency is calculated as the maximum relative distance from the centre.
...
> [!IMPORTANT]
> SIFT keypoint-based analysis is computationally expensive and can be significantly slower than FFT. It is recommended primarily for small source images where frequency analysis alone might be insufficient to capture complex structural features.
...
3.  **Area Weighting**: Each cluster centre (representing a characteristic scale) is paired with a weight proportional to the total area covered by keypoints in that cluster.
...
The outputs of the Cluster Size and Cluster Distance heuristics are merged into a single set of distance-weight pairs. These combined pairs are normalised and later combined with FFT-based pairs (if selected) in the synchronisation stage.

### 3. Synchronisation and Final Guess

The results from both analyses are filtered, normalised, and combined into a list of "Size-Weight Pairs" $(d, w)$.
...
2.  **Synchronisation Scoring**: For each candidate block size $b$, a cumulative "penalty score" is calculated. This score measures how well $b$ synchronises with all identified patterns $(d, w)$, weighted by their relative importance $w$:
    *   If $b < d$: $\text{penalty} = (d - b) \times w$ (heavily penalises block sizes smaller than the pattern).
    *   If $b \geq d$: $\text{penalty} = \min(b \pmod d, d - (b \pmod d)) \times w$ (measures how close $b$ is to being a multiple of $d$).
3.  **Selection**: The algorithm calculates the weighted sum of penalties across all patterns for each candidate. Those with the lowest cumulative penalties (highest synchronisation with the most dominant patterns) are identified as the best candidates.
4.  **Final Selection**: The heuristic returns the largest candidate among the best-synchronised values, thus minimising the total number of patches required for synthesis. This approach selects the most efficient option to optimise algorithm performance.
