...
> **Note:** A minimum `nps` of 1 always starts four processes, one per quadrant.
...
> **Note:** The `n_processes` parameter indicates the number of parallel processes to be run. The maximum value for `n_processes` is six, corresponding to the maximum number of directions and sectors that can be computed simultaneously.
